...
export default function ExpenseForm({
	...
	useAnimatedReaction(
		() => scrollOffset.value > SCROLL_THRESHOLD * 0.5,
		(collapsed, prev) => {
			if (collapsed !== prev) {
				scheduleOnRN(setIsCollapsed, collapsed);
			}
		},
	);
	...
	const collapsedAnimatedStyle = useAnimatedStyle(() => {
		...
		return {
			opacity: interpolate(progress, [0.5, 1], [0, 1], "clamp"),
		};
	});
	...
	return (
		...
		>
			<Animated.View style={[styles.amountContainer, containerAnimatedStyle]}>
				{/* Expanded: vertical centered layout */}
				<Animated.View
					style={[styles.expandedWrapper, expandedAnimatedStyle]}
				...
				{/* Collapsed: horizontal compact layout */}
				<Animated.View
					style={[styles.collapsedWrapper, collapsedAnimatedStyle]}
				...
				>
					...
					<View className="flex-row items-center">
						<Text className="mr-1 text-xl font-bold text-gray-800">
							{getCurrencySymbol(currency)}
						</Text>
						<RNTextInput
